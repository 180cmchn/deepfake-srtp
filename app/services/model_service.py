...
class ModelService:
    ...
    async def create_model(self, model: ModelCreate) -> ModelResponse:
        ...
        try:
            ...
            existing = (
                ...
                .filter(ModelRegistry.name == model.name, ModelRegistry.del_flag == 0)
                .first()
            )
            ...
            logger.info(
                "Model created", model_name=model.name, model_type=model.model_type
            )
    ...
    async def get_models(
        ...
        try:
            ...
            total = query.count()
            results = (
                query.order_by(ModelRegistry.created_at.desc())
                .offset(skip)
                .limit(limit)
                .all()
            )
    ...
    async def update_model(
        self, model_id: int, model_update: ModelUpdate
    ...
    async def deploy_model(
        self, model_id: int, deployment_config: Dict[str, Any]
    ) -> ModelDeployment:
        """Deploy model"""
        try:
            model = (
    ...
    async def get_statistics(self) -> ModelStatistics:
        ...
        try:
            # Get total models
            total_models = (
                self.db.query(ModelRegistry).filter(ModelRegistry.del_flag == 0).count()
            )
            ...
            for status in [
                ...
                count = (
                    self.db.query(ModelRegistry)
                    .filter(ModelRegistry.status == status, ModelRegistry.del_flag == 0)
                    .count()
                )
                if count > 0:
            ...
            models_with_accuracy = (
                self.db.query(ModelRegistry.accuracy)
                .filter(ModelRegistry.accuracy.isnot(None), ModelRegistry.del_flag == 0)
                .all()
            )
            average_accuracy = (
                sum(acc[0] for acc in models_with_accuracy) / len(models_with_accuracy)
            ...
            return ModelStatistics(
                ...
                recent_deployments=[
                    self._db_to_response(m) for m in recent_deployments
                ],
            )
    ...
    def _db_to_response(self, model: ModelRegistry) -> ModelResponse:
        ...
        status_value = model.status or ModelStatus.TRAINING.value
        try:
            status_enum = ModelStatus(status_value)
        except Exception:
            status_enum = ModelStatus.TRAINING

        return ModelResponse(
            id=model.id,
            name=model.name or f"model-{model.id}",
            model_type=model.model_type or "vgg",
            version=model.version or "1.0",
            description=model.description,
            input_size=model.input_size or 224,
            num_classes=model.num_classes or 2,
            parameters=model.parameters,
