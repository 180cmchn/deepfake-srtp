...
FAKE_LABEL_KEYWORDS = {"fake", "deepfake", "manipulated", "forged", "tampered", "class1", "1"}
REAL_LABEL_KEYWORDS = {"real", "authentic", "original", "genuine", "pristine", "class0", "0"}
...
class TrainingService:
    ...
    async def get_jobs(
        ...
        try:
            ...
            if created_by:
                logger.warning("created_by filter ignored: field not implemented", created_by=created_by)
    ...
    async def get_progress(self, job_id: int) -> Optional[TrainingProgress]:
        ...
        try:
            ...
            if current_epoch is None and job.status in [JobStatus.COMPLETED.value, JobStatus.FAILED.value, JobStatus.CANCELLED.value]:
    ...
    async def start_job(self, job_id: int, background_tasks: BackgroundTasks, current_user: str) -> bool:
        ...
        try:
            ...
            logger.info("Training job start scheduled", job_id=job_id, user=current_user)
            return True
        except Exception as e:
            logger.error("Failed to schedule training job", error=str(e), job_id=job_id, user=current_user)
            self.db.rollback()
            raise
    ...
    async def get_job_logs(self, job_id: int, tail_lines: int = 100) -> List[str]:
        ...
        try:
            ...
            if job.completed_at:
                lines.append(f"[{job.completed_at}] Training finished with status={job.status}")
    ...
    async def get_job_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            by_status = {
                status.value: self.db.query(TrainingJob).filter(
                    TrainingJob.del_flag == 0,
                    TrainingJob.status == status.value
                ).count()
            ...
            average_progress = (
                sum(row[0] for row in avg_progress_rows if row[0] is not None) / len(avg_progress_rows)
                if avg_progress_rows else 0.0
            )
    ...
    async def _run_training(self, job_id: int, parameters: Dict[str, Any]):
        ...
        try:
            ...
            trainer = self._train_sequence_model if model_type == "lrcn" else self._train_image_model
    ...
    def _train_image_model(self, job_id: int, parameters: Dict[str, Any]):
        ...
        train_loader, val_loader = self._build_dataloaders(dataset_path, batch_size, validation_split)

        model = create_model(model_type=model_type, num_classes=2).to(device)
        criterion = nn.CrossEntropyLoss()

        trainable_params = [p for p in model.parameters() if p.requires_grad]
        optimizer = torch.optim.Adam(trainable_params if trainable_params else model.parameters(), lr=learning_rate)
        ...
        for epoch in range(1, epochs + 1):
            if self._is_job_cancelled(job_id):
                raise TrainingCancelledError(f"Training job {job_id} cancelled at epoch {epoch}")

            train_loss, train_accuracy = self._run_train_epoch(model, train_loader, criterion, optimizer, device)
            val_loss, val_accuracy = self._run_eval_epoch(model, val_loader, criterion, device)

            progress = (epoch / epochs) * 100.0
            self._update_epoch_metrics(job_id, progress, val_accuracy, val_loss, epoch=epoch, total_epochs=epochs, message=f"Epoch {epoch}/{epochs} completed")
        ...
        self._finalize_successful_training(job_id, best_val_accuracy, best_val_loss, best_checkpoint_path)
    ...
    def _train_sequence_model(self, job_id: int, parameters: Dict[str, Any]):
        ...
        self._set_active_job_metadata(job_id, {"total_epochs": epochs, "mode": "sequence", "message": f"Preparing clips (sequence_length={sequence_length}, frame_stride={frame_stride})"})

        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        train_loader, val_loader = self._build_sequence_dataloaders(dataset_path, batch_size, validation_split, sequence_length, frame_stride)

        model = create_model(model_type=model_type, num_classes=2, input_size=25088, hidden_size=hidden_size, num_layers=num_layers).to(device)
        criterion = nn.CrossEntropyLoss()
        ...
        for epoch in range(1, epochs + 1):
            ...
            progress = (epoch / epochs) * 100.0
            self._update_epoch_metrics(job_id, progress, val_accuracy, val_loss, epoch=epoch, total_epochs=epochs, message=f"Sequence epoch {epoch}/{epochs} completed")

            logger.info("Sequence training epoch completed", job_id=job_id, epoch=epoch, total_epochs=epochs, train_loss=train_loss, train_accuracy=train_accuracy, val_loss=val_loss, val_accuracy=val_accuracy, progress=progress, sequence_length=sequence_length, frame_stride=frame_stride)

            if val_accuracy > best_val_accuracy:
                best_val_accuracy = val_accuracy
                best_val_loss = val_loss
                torch.save({"job_id": job_id, "model_type": model_type, "epoch": epoch, "model_state_dict": model.state_dict(), "optimizer_state_dict": optimizer.state_dict(), "val_accuracy": val_accuracy, "val_loss": val_loss, "input_size": settings.MODEL_INPUT_SIZE, "sequence_length": sequence_length, "frame_stride": frame_stride, "saved_at": datetime.now().isoformat()}, str(best_checkpoint_path))

        self._finalize_successful_training(job_id, best_val_accuracy, best_val_loss, best_checkpoint_path)

    def _finalize_successful_training(self, job_id: int, best_val_accuracy: float, best_val_loss: Optional[float], best_checkpoint_path: Path):
        ...
        self._set_active_job_metadata(job_id, {"status": JobStatus.COMPLETED.value, "current_epoch": None, "current_loss": best_val_loss, "current_accuracy": best_val_accuracy if best_val_accuracy >= 0 else None, "message": "Training completed"})

    def _build_dataloaders(self, dataset_path: str, batch_size: int, validation_split: float) -> Tuple[DataLoader, DataLoader]:
        ...
        if split_dirs:
            ...
            if len(all_samples) < 2:
                raise ValueError("Dataset must contain at least 2 labeled images for train/validation split")
        ...
        train_dataset = ImageClassificationDataset(train_samples, transform=train_transform)
    ...
    def _build_sequence_dataloaders(
        ...
        split_dirs = self._find_split_dirs(root_path)
        if split_dirs:
            train_videos = self._collect_labeled_video_groups(split_dirs["train"], split_dirs["train"])
            val_videos = self._collect_labeled_video_groups(split_dirs["val"], split_dirs["val"])
        else:
            all_videos = self._collect_labeled_video_groups(root_path, root_path)
            if len(all_videos) < 2:
                raise ValueError("Dataset must contain at least 2 labeled video groups for train/validation split")
        ...
        for frame_paths, label in train_videos:
            train_samples.extend(self._build_clips_from_video_frames(frame_paths, label, sequence_length, frame_stride))

        val_samples: List[Tuple[List[str], int]] = []
        for frame_paths, label in val_videos:
            val_samples.extend(self._build_clips_from_video_frames(frame_paths, label, sequence_length, frame_stride))
        ...
        train_dataset = SequenceClassificationDataset(train_samples, transform=train_transform)
        val_dataset = SequenceClassificationDataset(val_samples, transform=val_transform)
    ...
    def _collect_labeled_video_groups(self, root_path: Path, label_root: Path) -> List[Tuple[List[str], int]]:
    ...
    def _build_clips_from_video_frames(
        ...
        if len(frame_paths) < sequence_length:
            padded = frame_paths + [frame_paths[-1]] * (sequence_length - len(frame_paths))
            return [(padded, label)]

        if len(frame_paths) < clip_span:
            clip = [frame_paths[min(i * frame_stride, len(frame_paths) - 1)] for i in range(sequence_length)]
            return [(clip, label)]
    ...
    def _find_split_dirs(self, root_path: Path) -> Optional[Dict[str, Path]]:
        ...
        child_dirs = {child.name.lower(): child for child in root_path.iterdir() if child.is_dir()}

        train_dir = child_dirs.get("train")
        val_dir = child_dirs.get("val") or child_dirs.get("valid") or child_dirs.get("validation")

        if train_dir and val_dir:
            return {"train": train_dir, "val": val_dir}
        return None

    def _collect_labeled_images(self, root_path: Path, label_root: Path) -> List[Tuple[str, int]]:
