...
class DetectionVideoMetadataTruthfulnessTests(unittest.IsolatedAsyncioTestCase):
    async def test_detect_video_exposes_source_and_sampled_metadata_separately(self):
        ...
        self.assertEqual(
            response.summary["aggregation_strategy"], "weighted_topk_persistence"
        )
        self.assertEqual(response.summary["sampled_frame_count"], 2)
        self.assertEqual(response.summary["analyzed_frame_count"], 2)
        self.assertEqual(response.summary["fallback_filled_frame_count"], 0)
    ...
    async def test_detect_video_keeps_fallback_filled_frames_out_of_analyzed_count(
        self,
    ):
        service = DetectionService(db=None)
        loaded_model = {
        ...
        saved_record = SavedDetectionRecord(id=89, created_at=datetime.now())
        ...
        try:
            with (
                ...
                patch.object(
                    ...
                    "_extract_frames",
                    new=AsyncMock(
                        return_value=[(object(), 0.0), (object(), 1.0), (object(), 2.0)]
                    ),
                ),
                patch.object(
                    service,
        ...
        self.assertTrue(response.success)
        self.assertEqual(response.video_info["sampled_frame_count"], 3)
        self.assertEqual(response.video_info["analyzed_frame_count"], 2)
        self.assertEqual(response.video_info["fallback_filled_frame_count"], 1)
        self.assertAlmostEqual(response.summary["success_rate"], 2 / 3)
        self.assertEqual(response.summary["processed_frames"], 2)
        self.assertEqual(response.summary["fallback_filled_frame_count"], 1)
        await_args = save_mock.await_args
        if await_args is None:
            self.fail("_save_detection_result should be awaited with truthful counts")
        self.assertEqual(await_args.kwargs["video_metadata"]["sampled_frame_count"], 3)
        self.assertEqual(await_args.kwargs["video_metadata"]["analyzed_frame_count"], 2)
